...
public class CoverEyemovement : MonoBehaviour
{
    ...
    {
        if (other.gameObject.CompareTag("eye_L") && CoverInit.position == 1)
        {
            try_number += 1;

            if (CoverInit.type == 0) // 우안 내사시
            {
                switch (CoverInit.angle)
                {
                    case 10: // 우안 내사시 10도
        ...
        if (other.gameObject.CompareTag("eye_R") && CoverInit.position == 0)
        {
            try_number += 1;

            if (CoverInit.type == 0) // 좌안 내사시
            {
                switch (CoverInit.angle)
                {
                    case 10: // 좌안 내사시 10도
